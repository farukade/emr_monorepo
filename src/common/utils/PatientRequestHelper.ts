...
export class PatientRequestHelper {
  ...
  static async handleVaccinationRequest(param, patient, createdBy) {
    ...
    const vaccines = await getConnection().getRepository(Immunization).find({ date_due, patient });
  ...
  static async handleServiceRequest(param, patient, createdBy, type, visit = '', encounter = null) {
    ...
    try {
      ...
      const nextId = `00000${serialCode}`;
      const code = `${requestType.toUpperCase().substring(0, 1)}R${moment().format('YY')}/${moment().format(
        'MM',
      )}/${nextId.slice(-5)}`;
      ...
      for (const item of tests) {
        if (item && item.code) {
          ...
          const requestItem: any = {
            ...
            service,
            createdBy,
            can_schedule: procedure_type ? procedure_type === 'schedule' : true,
          };
  ...
  static async save(data): Promise<any> {
    return await getConnection().createQueryBuilder().insert().into(PatientRequest).values(data).returning('*').execute();
  }

  static async update(data, id): Promise<any> {
    return await getConnection().createQueryBuilder().update(PatientRequest).set(data).where('id = :id', { id }).execute();
  }
}
