...
export class PatientRequestHelper {
	...
	static async handleLabRequest(
		...
		try {
			...
			const code = `LR${moment().format('YY')}/${moment().format(
				'MM',
			)}/${nextId.slice(-5)}`;
			...
			if (antenatal_id && antenatal_id !== '') {
				antenatal = await getConnection()
					...
					.findOne(antenatal_id);
			}

			let admission = null;
			if (admission_id && admission_id !== '') {
				admission = await getConnection()
					.getRepository(Admission)
					.findOne(admission_id);
			}
			...
			for (const item of tests) {
				...
				const labTest = await getConnection()
	...
	static async handlePharmacyRequest(
		param: any,
		patient: Patient,
		createdBy: string,
		visit = '',
		...
		try {
			const serialCode = await getSerialCode(requestType);
			const nextId = `00000${serialCode}`;
			const code = `DR${moment().format('YY')}/${moment().format(
				'MM',
			)}/${nextId.slice(-5)}`;
			...
			for (const item of items) {
				const data = {
					...
					nicu,
					procedure_id:
						procedure_id && procedure_id !== '' ? procedure_id : null,
					encounter,
				};
				const res = await this.save(data);
				...
				const generic = item.generic
					? await getConnection()
							...
							.findOne(item.generic?.id)
					: null;
				const drug = item.drug
					? await getConnection()
							.getRepository(Drug)
							.findOne(item.drug?.id)
					: null;
	...
	static async handleVaccinationRequest(param, patient, createdBy) {
		...
		const nextId = `00000${serialCode}`;
		const code = `PR${moment().format('YY')}/${moment().format(
			'MM',
		)}/${nextId.slice(-5)}`;
	...
	static async handleServiceRequest(
		param,
		patient,
		createdBy,
		type,
