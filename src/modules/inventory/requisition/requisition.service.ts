...
export class RequisitionService {
	...
	async fetchAll(
		...
		if (status && status !== '') {
			if (status === 'pending') {
				query
					.andWhere('r.approved = :status', { status: 0 })
		...
		for (const item of requisitions) {
			if (item.store_inventory_id) {
				item.store = await this.storeInventoryRepository.findOne(
					item.store_inventory_id,
				);
			}

			if (item.cafeteria_inventory_id) {
				item.cafeteria = await this.cafeteriaInventoryRepository.findOne(
					item.cafeteria_inventory_id,
				);
			}
			...
			if (item.department_id) {
				item.department = await this.departmentRepository.findOne(
					item.department_id,
				);
			}
	...
	async approve(id, username: string): Promise<any> {
		try {
			...
			if (item.category === 'store') {
				const store = await this.storeInventoryRepository.findOne(
					item.store.id,
				);
				store.quantity = store.quantity - item.quantity;
				await store.save();
			} else if (item.category === 'cafeteria') {
				const cafeteria = await this.cafeteriaInventoryRepository.findOne(
					item.cafeteria.id,
				);
