...
export class DrugController {
	...
	create(@Body() drugDto: DrugDto, @Request() req): Promise<any> {
		return this.drugService.create(drugDto, req.user.username);
	}

	@Put('/:id')
	@UsePipes(ValidationPipe)
	update(@Param('id') id: string, @Body() drugDto: DrugDto, @Request() req): Promise<any> {
	...
	@Get('report')
	getDrugsReport(@Query() urlParams) {
		return this.drugService.getDrugsReport(urlParams);
	}
}
