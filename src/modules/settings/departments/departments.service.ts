...
export class DepartmentsService {
	...
	async getDepartments(): Promise<Department[]> {
		const results = await this.departmentRepository
	...
	async createDepartment(
		...
		return this.departmentRepository.saveDepartment(
			departmentDto,
			staff,
			createdBy,
		);
	...
	async updateDepartment(
		...
		try {
			...
			if (hod_id && hod_id !== '') {
				...
				if (
					dept &&
					dept.id !== parseInt(id, 10) &&
					dept.staff.id === parseInt(hod_id, 10)
				) {
					return {
						success: false,
						message: 'you cannot use the same staff for multiple departments',
					};
				}
			}
