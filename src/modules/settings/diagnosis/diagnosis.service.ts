  ...
  async getAllDiagnosis({ pagesize, page, q, alphabet }): Promise<DiagnosisPaginationDto> {
    ...
    if (alphaBet === '') {
      ...
      if (search === '') {
        ...
        [diagnoses, total] = await this.diagnosisRepository
          .createQueryBuilder('d')
          .where('LOWER(d.code) Like :alphabet', { alphabet: `${alphaBet.toLowerCase()}%` })
          .andWhere(
            new Brackets((qb) => {
              qb.where('LOWER(d.code) Like :code', { code: `%${search.toLowerCase()}%` }).orWhere(
                'LOWER(d.description) Like :description',
  ...
  async findDiagnosis(urlParam): Promise<Diagnosis[]> {
    ...
    if (diagnosisType && diagnosisType !== '') {
      return this.diagnosisRepository
        ...
        .andWhere(
          new Brackets((qb) => {
            qb.where('UPPER(d.code) Like :code', { code: `%${search.toUpperCase()}%` }).orWhere(
              'LOWER(d.description) Like :description',
              { description: `%${search.toLowerCase()}%` },
            );
          }),
