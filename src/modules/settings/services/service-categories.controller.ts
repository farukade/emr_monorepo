...
export class ServicesCategoryController {
  ...
  @Get('')
  getCategories(@Query() urlParams): Promise<ServiceCategory[]> {
    return this.servicesService.getServicesCategory(urlParams);
  }

  @Get('/:slug')
  getServicesCategoryBySlug(@Param('slug') slug: string): Promise<ServiceCategory> {
  ...
  @Patch('/:id')
  @UsePipes(ValidationPipe)
  updateCategory(@Param('id') id: number, @Body() serviceCategoryDto: ServiceCategoryDto): Promise<ServiceCategory> {
    return this.servicesService.updateServiceCategory(id, serviceCategoryDto);
  }

  @Delete('/:id')
  deleteServiceCategory(@Param('id') id: number, @Request() req): Promise<any> {
