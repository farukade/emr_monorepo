...
export class AntenatalPackageController {
  constructor(private packageService: AntenatalPackageService) {}

  @Get('')
  getPackages(@Query() urlParams, @Request() request): Promise<Pagination> {
  ...
  savePackage(@Body() createDto: AntenatalPackageDto, @Request() req) {
    return this.packageService.savePackage(createDto, req.user.username);
  }

  @Patch('/:id')
  @UsePipes(ValidationPipe)
  updatePackage(@Param('id') id: number, @Body() updateDto: AntenatalPackageDto, @Request() req) {
    return this.packageService.updatePackage(id, updateDto, req.user.username);
  }

  @Delete('/:id')
  deletePackage(@Param('id') id: number, @Request() req): Promise<AntenatalPackage> {
