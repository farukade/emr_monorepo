...
export class LabController {
   ...
   */
  @Get()
  getLabTests(@Request() request, @Query() urlParams): Promise<Pagination> {
  ...
  @Get('/unpaginated')
  getLabTestsUnPaginated(@Request() request, @Query() urlParams): Promise<any> {
    return this.labService.getTestsUnpaginated(urlParams);
  }

  @Post()
  @UsePipes(ValidationPipe)
  createLabTest(@Body() labTestDto: LabTestDto, @Request() req): Promise<any> {
    return this.labService.createLabTest(labTestDto, req.user.username);
  }

  @Patch(':id')
  @UsePipes(ValidationPipe)
  updateLabTest(@Param('id') id: string, @Body() labTestDto: LabTestDto, @Request() req): Promise<any> {
    return this.labService.updateLabTest(id, labTestDto, req.user.username);
  }

  @Delete(':id')
  deleteLabTest(@Param('id') id: number, @Request() req): Promise<any> {
   ...
   */
  @Get('/categories')
  getCategories(@Query() param): Promise<any[]> {
    return this.labService.getCategories(param);
  }

  @Post('/categories')
  @UsePipes(ValidationPipe)
  createLabTestCategory(@Body() labCategoryDto: LabCategoryDto, @Request() req): Promise<LabTestCategory> {
    return this.labService.createCategory(labCategoryDto, req.user.username);
  }

  @Put('categories/:id')
  @UsePipes(ValidationPipe)
  updateCategory(@Param('id') id: number, @Body() labCategoryDto: LabCategoryDto, @Request() req): Promise<LabTestCategory> {
    return this.labService.updateCategory(id, labCategoryDto, req.user.username);
  }

  @Delete('categories/:id')
  deleteServiceCategory(@Param('id') id: number, @Request() req): Promise<LabTestCategory> {
   ...
   */
  @Get('/parameters')
  getParameters(@Query('q') q: string): Promise<LabTestCategory[]> {
  ...
  @Patch('parameters/:id')
  @UsePipes(ValidationPipe)
  updateParameter(@Param('id') id: string, @Body() parameterDto: ParameterDto): Promise<Parameter> {
    return this.labService.updateParameter(id, parameterDto);
  }

  @Delete('parameters/:id')
  deleteParameter(@Param('id') id: number, @Request() req): Promise<Parameter> {
  ...
  @Post('/specimens')
  @UsePipes(ValidationPipe)
  createSpecimen(@Body() specimenDto: SpecimenDto, @Request() req): Promise<Specimen> {
    return this.labService.createSpecimen(specimenDto, req.user.username);
  }

  @Patch('/specimens/:id')
  @UsePipes(ValidationPipe)
  updateSpecimen(@Param('id') id: string, @Body() specimenDto: SpecimenDto, @Request() req): Promise<Specimen> {
    return this.labService.updateSpecimen(id, specimenDto, req.user.username);
  }

  @Delete('/specimens/:id')
  deleteSpecimen(@Param('id') id: number, @Request() req): Promise<Specimen> {
   ...
   */
  @Get('/groups')
  getGroups(@Query() param): Promise<any[]> {
    return this.labService.getGroups();
  }

  @Post('/groups')
  @UsePipes(ValidationPipe)
  createGroup(@Body() specimenDto: GroupDto, @Request() req): Promise<any> {
    return this.labService.createGroup(specimenDto, req.user.username);
  }

  @Patch('/groups/:id')
  @UsePipes(ValidationPipe)
  updateGroup(@Param('id') id: number, @Body() groupDto: GroupDto, @Request() req): Promise<any> {
    return this.labService.updateGroup(id, groupDto, req.user.username);
  }

  @Delete('/groups/:id')
  deleteGroup(@Param('id') id: number, @Request() req): Promise<Group> {
