...
export class DoctorsAppointmentService {
  ...
  async getDoctorsAppointments(params: any): Promise<DoctorsAppointment[]> {
    ...
    const query = this.doctorsAppointmentRepository.createQueryBuilder('d').select('d.*');
  ...
  async checkDate(param): Promise<any> {
    try {
      ...
      const end = moment(date, 'YYYY-MM-DD HH:mm:ss').add(duration, duration_type).format('YYYY-MM-DD HH:mm:ss');
