...
export class PatientAllergenController {
  constructor(private allergenService: PatientAllergenService) {}

  @Get('')
  getAllergies(@Query() urlParams, @Request() request): Promise<any> {
  ...
  saveAllergies(@Body() param: PatientAllergyDto, @Request() req) {
    return this.allergenService.doSaveAllergies(param, req.user.username);
  }

  @Patch(':id/update')
  @UsePipes(ValidationPipe)
  updateAllergy(@Param('id') id: number, @Body() param: PatientAllergyDto, @Request() req) {
    return this.allergenService.doUpdateAllergy(id, param, req.user.username);
  }

  @Delete('/:id')
  deletePackage(@Param('id') id: number, @Request() req): Promise<PatientNote> {
