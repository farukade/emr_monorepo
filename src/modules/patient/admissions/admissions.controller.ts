...
export class AdmissionsController {
  ...
  @Get()
  getAdmissions(@Query() urlParams, @Request() request): Promise<Pagination> {
  ...
  saveNewAdmission(@Param('id') id: string, @Body() createDto: CreateAdmissionDto, @Request() req): Promise<any> {
    return this.admissionService.saveAdmission(id, createDto, req.user.username);
  }

  @Put(':id/start-discharge')
  @UsePipes(ValidationPipe)
  startDischarge(@Param('id') id: number, @Body() params, @Request() req): Promise<any> {
  ...
  @Patch('/assign-bed')
  assignBed(@Body() params, @Request() req) {
    return this.admissionService.saveAssignBed(params, req.user.username);
  }

  @Get('/tasks')
  getTasks(@Query() urlParams, @Request() request) {
  ...
  deleteTask(@Param('taskId') taskId: number, @Request() req): Promise<any> {
    return this.admissionService.deleteTask(taskId, req.user.username);
  }

  @Post('/create-task/:id')
  createTask(@Param('id') id: number, @Body() params, @Request() req) {
    return this.admissionService.saveClinicalTasks(id, params, req.user.username);
  }

  @Get('ward-rounds')
  getWardRounds(@Query() urlParams, @Request() request) {
  ...
  @Post('/soap')
  saveSoap(@Body() param: SoapDto, @Request() req) {
