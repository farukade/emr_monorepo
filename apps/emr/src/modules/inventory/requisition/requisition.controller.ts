...
export class RequisitionController {
  ...
  create(@Body() requisitionDto: RequisitionDto, @Request() req): Promise<any> {
    return this.requisitionService.create(requisitionDto, req.user.username);
  }

  @Put('/:id')
  @UsePipes(ValidationPipe)
  update(@Param('id') id: string, @Body() requisitionDto: RequisitionDto, @Request() req): Promise<any> {
    return this.requisitionService.updateItem(id, requisitionDto, req.user.username);
  }

  @Put('/:id/approve')
  approve(@Param('id') id: number, @Request() req): Promise<any> {
    return this.requisitionService.approve(id, req.user.username);
  }

  @Put('/:id/decline')
  decline(@Param('id') id: number, @Body() param, @Request() req): Promise<any> {
    return this.requisitionService.decline(id, param, req.user.username);
  }

  @Delete('/:id')
  deleteItem(@Param('id') id: number, @Request() req): Promise<any> {
