...
export class CafeteriaInventoryService {
  ...
  async updateQty(id, inventoryDto: InventoryDto, username: string): Promise<any> {
    try {
      const { quantity, unit_price, vendor_id, vendor_label, selling_price } = inventoryDto;
      ...
      await this.inventoryActivityRepository.saveActivity({ cafeteria: rs, quantity, unitPrice: item.unitPrice }, username);
