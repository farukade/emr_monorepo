...
export class NicuAccommodationController {
  constructor(private accommodationService: NicuAccommodationService) {}

  @Get('')
  getAccommodations(@Query() urlParams, @Request() request): Promise<Pagination> {
  ...
  saveAccommodation(@Body() createDto: AccommodationDto, @Request() req) {
    return this.accommodationService.saveAccommodation(createDto, req.user.username);
  }

  @Patch('/:id')
  @UsePipes(ValidationPipe)
  updateAccommodation(@Param('id') id: number, @Body() updateDto: AccommodationDto, @Request() req) {
    return this.accommodationService.updateAccommodation(id, updateDto, req.user.username);
  }

  @Delete('/:id')
  deleteAccommodation(@Param('id') id: number, @Request() req): Promise<NicuAccommodation> {
