...
export class DepartmentsController {
  ...
  createDepartment(@Body() departmentDto: DepartmentDto, @Request() req): Promise<Department> {
    return this.departmentService.createDepartment(departmentDto, req.user.username);
  }

  @Patch('/:id/update')
  @UsePipes(ValidationPipe)
  updateDepartment(@Param('id') id: string, @Body() departmentDto: DepartmentDto, @Request() req): Promise<any> {
    return this.departmentService.updateDepartment(id, departmentDto, req.user.username);
  }

  @Delete('/:id')
  deletePackage(@Param('id') id: number, @Request() req): Promise<Department> {
