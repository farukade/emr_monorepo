...
export class VouchersController {
  ...
  @Get('list')
  getVouchers(@Query() urlParams, @Request() request): Promise<Pagination> {
  ...
  @Get(':code')
  getVoucher(@Param('code') code: string): Promise<Voucher> {
    return this.voucherService.fetchByCode(code);
  }

  @Post('')
  @UsePipes(ValidationPipe)
  saveVoucher(@Body() voucherDto: VoucherDto, @Request() req): Promise<any> {
    return this.voucherService.save(voucherDto, req.user.username);
  }

  @Patch('/:id/update')
  @UsePipes(ValidationPipe)
  updateVoucher(@Param('id') id: string, @Body() voucherDto: VoucherDto, @Request() req): Promise<any> {
